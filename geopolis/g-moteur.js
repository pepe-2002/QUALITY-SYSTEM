...
(function (G) {
  ...
  G.nouvellePartie = function (codeJoueur, difficulte) {
    ...
    for (let i = 0; i < N; i++) {
      const p = G.PAYS[i];
      E.echelle[i] = clamp(Math.pow(Math.max(p.pop / 1e6, 0.05) / 12, 0.85), 0.05, 60);
      E.pop[i] = p.pop;
      E.pib[i] = p.pib;
      E.tech[i] = p.tech;
      E.stab[i] = p.stab;
      ...
      const pibHab0 = p.pib * 1e9 / Math.max(p.pop, 1);
      const pauvrete = clamp(1 - pibHab0 / 20000, 0, 1);
      E.dette[i] = p.pib * 1000 * (0.30 + Math.random() * 0.45) * (1 - pauvrete * 0.45);
  ...
  function dotationsInitiales(i, p) {
    ...
    const pop = p.pop / 1e6;
    // Les quantités ci-dessous décrivent une capacité totale ; on la convertit
    // en nombre de bâtiments à la taille du pays.
    const t = E.echelle[i];
    const set = (b, n) => { E.bat[i * NBAT + b] = Math.max(0, Math.round(n / t)); };
  ...
  function calculerFlux() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (let b = 0; b < NBAT; b++) {
        ...
        const q = n * taille;                       // capacité, et non nombre d'unités
        const pp = BPROD[b], cc = BCONSO[b];
        for (let k = 0; k < pp.length; k += 2) E.prod[base + pp[k]] += pp[k + 1] * q * m * facteur;
        for (let k = 0; k < cc.length; k += 2) E.conso[base + cc[k]] += cc[k + 1] * q;
        if (bd.rech)   rechJour += bd.rech * q;
        if (bd.educ)   educB += bd.educ * q;
        if (bd.sante)  santeB += bd.sante * q;
        if (bd.infra)  infraB += bd.infra * q;
        if (bd.export) expB += bd.export * q;
        if (bd.milcap) milB += bd.milcap * q;
      }
  ...
  function economie() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      // ── Croissance
      const capital = clamp((E.prod[base + R.biens] / Math.max(E.conso[base + R.biens], 1) - 1) * 0.8, -0.5, 0.9);
      const rattrapage = clamp((1 - pibHab / pibMax) * 1.6, 0, 1.6);
      // pibHab est ici en milliers de dollars : 70 vaut 70 000 $ par habitant.
      const frontiere = clamp((pibHab / 70 - 0.6) * 3, 0, 8);
  ...
  function tauxInteret(i) {
    ...
    // Les pays à faible revenu empruntent surtout auprès des bailleurs
    // multilatéraux, à des taux sans rapport avec ceux du marché. Sans cette
    // distinction, aucune petite nation ne pouvait équilibrer ses comptes.
    const pibHab = E.pib[i] * 1e9 / Math.max(E.pop[i], 1);
    const concession = 1 - clamp(1 - pibHab / 6000, 0, 1) * 0.55;
    return clamp((0.018 + ratio * 0.045 + (100 - E.stab[i]) * 0.0005) * concession, 0.008, 0.30);
  }
  ...
  function coutEntretien(i) {
    let c = 0;
    const t = E.echelle[i];
    for (let b = 0; b < NBAT; b++) c += E.bat[i * NBAT + b] * G.BAT[b].entretien * t;
  ...
  function societe() {
    for (let i = 0; i < N; i++) {
      ...
      // On ne juge pas un gouvernement à l'aune d'une norme universelle : un
      // peuple attend de son État ce que le pays peut raisonnablement offrir.
      // Sans cette échelle d'attentes, une nation pauvre partait avec vingt-huit
      // points de malus permanents — services, chômage et corruption jugés à
      // l'aune d'un pays riche — et sombrait quoi que fasse le joueur.
      const pibHab = E.pib[i] * 1e9 / Math.max(E.pop[i], 1);
      const attServices = clamp(20 + 60 * Math.sqrt(Math.min(pibHab / 40000, 1.4)), 20, 86);
      const attChomage  = clamp(6 + (1 - pibHab / 30000) * 8, 6, 14);
      const attCorrupt  = clamp(20 + (1 - pibHab / 25000) * 45, 20, 65);

      let cibleApp = 46
        + clamp((E.croiss[i] - 1.5) * 5.5, -20, 20)
        - clamp((E.chomage[i] - attChomage) * 1.5, -10, 24)
        - clamp((E.inflation[i] - 3) * 1.6, -8, 26)
        - (E.tIR[i] * 42 + E.tTVA[i] * 34 + E.tIS[i] * 10 - 22)
        + (E.sante[i] - attServices) * 0.16
        + (E.educ[i] - attServices) * 0.11
        + (E.infra[i] - attServices) * 0.07
        - E.corrupt[i] * 0.10
        - clamp(E.corrupt[i] - attCorrupt, -20, 40) * 0.22
  ...
  function coutBatiment(i, b) {
    ...
    return G.BAT[b].cout * E.echelle[i] * (1 + n * 0.012) * (1 + E.inflation[i] / 260);
  }
  ...
  G.sauver = function () {
    ...
    ['pop','pib','croiss','tresor','dette','tech','stab','approb','corrupt','chomage','inflation',
     ...
     'tIR','tIS','tTVA','tDou','bSante','bEduc','bInfra','bDef','bSub','prix','prixBase','calib','echelle',
