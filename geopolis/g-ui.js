...
(function (G) {
  ...
  function majDetail() {
    ...
    if (!enGuerre) {
      act.appendChild(bouton('Déclarer la guerre', 'danger', () => {
        if (!confirm(`Déclarer la guerre à ${p.nom} ? Rapport de force estimé : ${nf2.format(rapport)} ×.`)) return;
      ...
      mes.forEach(g => {
        ...
        if (E.uni[i * G.NUNI + U.ogive] > 0) {
          a.appendChild(bouton('☢️ Frappe nucléaire', 'danger', () => {
            if (!confirm(`Frapper ${G.PAYS[adv].nom} à l'arme nucléaire ? Des millions de morts, votre réputation détruite, et une riposte probable.`)) return;
  ...
  function modaleFin() {
    ...
    modale(`<div class="mo-i">${f.type === 'victoire' ? '🏆' : '💀'}</div>
      ...
      [['Nouvelle partie', () => location.reload(), 'principal']], 'fin');
  }
